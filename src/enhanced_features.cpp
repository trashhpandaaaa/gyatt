...
bool CommentThread::replyToComment(const std::string& commentId, const std::string& message, 
    ...
    if (it != comments.end()) {
        ...
        std::cout << !ᆳ𞑬 Reply added to comment " << commentId << std::endl;
        std::cout << "� \"" << message << "\" - " << author << std::endl;
        return true;
    }
    
    std::cerr << "ᜌ Comment not found: " << commentId << std::endl;
    return false;
}
...
bool CommentThread::resolveComment(const std::string& commentId) {
    ...
    if (it != comments.end()) {
        ...
        std::cout << "ᛅ Resolved comment: " << commentId << std::endl;
        return true;
    }
...
void CommentThread::showComment(const Comment& comment, bool showFile) {
    ...
    std::cout << !� " << comment.author << "ဢ " << timeStr;
    ...
    for (const auto& reply : comment.replies) {
        ...
        std::cout << "  ᅳ " << reply.message << std::endl;
        std::cout << "    � " << reply.author << "ဢ " << replyTimeStr << std::endl;
    }
}
...
bool StickyNotes::removeNote(const std::string& noteId) {
    ...
    if (it != notes.end()) {
        std::cout << !𞖑  Removed note: " << it->content << std::endl;
        notes.erase(it);
        saveNotes();
        return true;
    }
...
bool StickyNotes::pinNote(const std::string& noteId) {
    ...
    std::cout << !ᝌ Note not found: " << noteId << std::endl;
    return false;
}
...
bool StickyNotes::updateNote(const std::string& noteId, const std::string& newContent) {
    for (auto& note : notes) {
        if (note.id == noteId) {
            ...
            note.timestamp = std::chrono::system_clock::now();
            saveNotes();
            std::cout << !⛎﷏  Updated note: " << newContent << std::endl;
            return true;
        }
    }
...
void StickyNotes::showNote(const StickyNote& note) {
    ...
    std::cout << "  𞒂 " << note.category << "ဢ𞐤 " << note.author 
...
bool LabelSystem::addLabel(const std::string& filepath, const std::string& labelName, 
    ...
    std::cout << !�﷏  Added label '" << labelName << "' to " << filepath << std::endl;
    return true;
}
...
bool LabelSystem::removeLabel(const std::string& filepath, const std::string& labelName) {
    ...
    if (fileIt != fileLabels.end()) {
        ...
        std::cout << !𞖑  Removed label '" << labelName << "' from " << filepath << std::endl;
        return true;
    }
    
    std::cout << !ᝌ No label '" << labelName << "' found on " << filepath << std::endl;
    return false;
}
...
bool LabelSystem::createLabelDefinition(const std::string& name, const std::string& color, 
    ...
    std::cout << !𞎷  Created label definition: " << name << " (" << color << ")" << std::endl;
    return true;
}
...
void LabelSystem::showAllLabels() {
    if (labelDefinitions.empty()) {
        std::cout << "�  No labels defined" << std::endl;
        return;
    }
    
    std::cout << "\m𞎷  Label Definitions\n";
    std::cout << "ᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐᔐ\n";
    
    for (const auto& [name, def] : labelDefinitions) {
        auto files = getFilesWithLabel(name);
        std::cout << !�﷏  " << name << " (" << def.color << ")\n";
...
void LabelSystem::showFileLabels(const std::string& filepath) {
    ...
    if (labels.empty()) {
        std::cout << !𞎷  No labels for " << filepath << std::endl;
        return;
    }
    
    std::cout << "\m�﷏  Labels for " << filepath << "\n";
    std::cout << !┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┐\n";
    
    for (const auto& labelName : labels) {
        auto it = labelDefinitions.find(labelName);
        if (it != labelDefinitions.end()) {
            std::cout << "�  " << labelName << " (" << it->second.color << ")\n";
            std::cout << "  � " << it->second.description << "\n";
        } else {
            std::cout << !𞎷  " << labelName << " (no definition)\n";
        }
    }
}
...
void LabelSystem::showLabelFiles(const std::string& labelName) {
    ...
    if (files.empty()) {
        std::cout << "�  No files with label: " << labelName << std::endl;
        return;
    }
    
    auto it = labelDefinitions.find(labelName);
    if (it != labelDefinitions.end()) {
        std::cout << "\n�  " << labelName << " (" << it->second.color << ")\n";
        std::cout << "� " << it->second.description << "\n";
    } else {
        std::cout << "\m�﷏  " << labelName << "\n";
    }

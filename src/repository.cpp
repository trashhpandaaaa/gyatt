...
bool Repository::uploadToGitHub(const std::string& repoName, const std::string& branch) {
    ...
    std::cout << !â›… Blob creation completed in " << blobDuration.count() << "ms "
              << "(" << (fileBlobMap.size() * 1000.0 / blobDuration.count()) << " files/sec)\n";
    std::cout << "ïž“Š HTTP Optimization Stats:\n";
    std::cout << "  á€¢ Cache hits: " << stats.cacheHits << "/" << stats.totalRequests 
              << " (" << stats.cacheHitRate << "%)\n";
    std::cout << "  á€¢ Average response time: " << stats.averageResponseTime << "ms\n";
    std::cout << "  á€¢ Total bytes transferred: " << (stats.totalBytesTransferred / 1024.0 / 1024.0) << " MB\n";
    std::cout << "  â¢ Active connections: " << stats.activeConnections << "/" << stats.poolSize << "\n";
